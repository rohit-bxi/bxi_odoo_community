...
export class SignTemplate extends Component {
    ...
    updateRoleName(roleId, roleName) {
        // Chain writes per role so overlapping renames (e.g. triggered in quick
        // succession by deleteSigner/updateSignerNames) never issue concurrent
        // UPDATE statements on the same sign.item.role row, which causes
        // PostgreSQL "could not serialize access due to concurrent update" errors.
        this.roleNameWriteQueues ??= new Map();
        const previousWrite = this.roleNameWriteQueues.get(roleId) || Promise.resolve();
        const write = previousWrite
            .catch(() => {})
            .then(() => this.orm.write("sign.item.role", [roleId], { name: roleName }));
        this.roleNameWriteQueues.set(roleId, write);

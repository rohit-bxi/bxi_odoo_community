...
class TravelRequestPortal(http.Controller):
    ...
    def travel_request_list(self, filter_type=None, **kwargs):
        ...
        T_Request = request.env['mybiz.travel.request'].sudo()
        ...
        return request.render('bxi_mybiz_travel.bxi_travel_request_template', {
            'requests': requests,
            'employee': employee,
            'is_hr': is_hr,
    ...
    def submit_request(self, **post):
        ...
        if request.httprequest.method == 'POST' and post.get('travel_purpose'):
            ...
            rec = request.env['mybiz.travel.request'].sudo().create(vals)

            # Auto-create a travel segment line based on primary travel selection
            if post.get('mode_of_travel'):
                request.env['mybiz.travel.option'].sudo().create({
        ...
        return request.render('bxi_mybiz_travel.submit_travel_template', {
            'employee': employee,
            ...
            'mode_options': request.env['mybiz.travel.request']._fields['mode_of_travel'].selection,
            'trip_options': request.env['mybiz.travel.request']._fields['trip_type'].selection,
            'class_options': request.env['mybiz.travel.request']._fields['travel_class'].selection,
            'hotel_grade_options': request.env['mybiz.travel.request']._fields['hotel_grade'].selection,
            'cab_type_options': request.env['mybiz.travel.request']._fields['cab_type'].selection,
        })
    ...
    def travel_request_detail(self, rec_id, **kwargs):
        record = request.env['mybiz.travel.request'].sudo().browse(rec_id)
        ...
        return request.render('bxi_mybiz_travel.travel_request_detail_template', {
            'record': record,

...
class TravelRequest(models.Model):
    _name = 'mybiz.travel.request'
    _description = 'myBiz Travel Request'
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('mybiz.travel.request') or 'BXI/TR/0001'
    ...
    def _push_to_mybiz(self):
        ...
        self.ensure_one()
        config = self.env['mybiz.config'].get_active_config(self.company_id.id)
        if not config:
            self.write({
                'mybiz_status': 'failed',
            ...
            self.message_post(body=_(
                ...
                'Please go to myBiz Travel → Configuration and set up the API credentials.'
    ...
    def _send_state_email(self):
        ...
        if self.state == 'manager_approval':
            template = self.env.ref('bxi_mybiz_travel.email_template_manager', raise_if_not_found=False)
            email_to = self.employee_id.parent_id.work_email if self.employee_id.parent_id else False
        elif self.state == 'hr_approval':
            template = self.env.ref('bxi_mybiz_travel.email_template_hr', raise_if_not_found=False)
            email_to = 'hr@bxitech.com'
        elif self.state == 'approved':
            template = self.env.ref('bxi_mybiz_travel.email_template_approved', raise_if_not_found=False)
